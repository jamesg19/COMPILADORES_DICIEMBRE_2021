...
export class Sqrt extends Instruccion{
    ...
    traducir(entorno: TablaSimbolos, arbol: Arbol): any {
        try {
            ...
            if(izq instanceof Excepcion){
                ...
                return new Excepcion("Semantico",`Tipo de datos invalido para Sqrt()  ${this.operadorIzq.tipo}`,`${this.fila}`,`${this.columna}`);

        } catch (error) {

            return new Excepcion("Semantico","QUETZAL Null Poiter Sqrt() tipo dato incorrecto ",`${this.fila}`,`${this.columna}`);
    ...
    setAtributosC3D(izquierda:string,derecha:string){
        ...
        let t = "t"+temp;
        Principal.temp = temp;
        Principal.historial += t +" = sqrt("+izquierda+");" ;
        Principal.historial += "\n";
        this.tipo = TIPO.DECIMAL;
        return t; 
    }
